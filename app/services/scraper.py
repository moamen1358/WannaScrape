...
# ============================================================================
# CAPTCHA SOLVER - Supports 2captcha, capsolver for various CAPTCHA types
# ============================================================================
...
class CaptchaSolver:
    ...
    def solve(self, page: Page, url: str) -> bool:
        ...
        Note: PerimeterX (Press & Hold) solving works even without API key
        since it's behavioral-based.
        """
    ...
    def _poll_2captcha_result(self, client: httpx.Client, task_id: str) -> Optional[str]:
        ...
        while time.time() - start_time < self.timeout:
            time.sleep(5)  # 2captcha recommends 5s intervals
            ...
            page.evaluate(f"""
                () => {{
                    ...
                    if (typeof turnstile !== 'undefined' && turnstile.render) {{
                        // Turnstile callback is usually set during render
                    }}
                }}
            """)
    ...
    def _solve_with_capsolver(self, page: Page, url: str, captcha_info: dict) -> bool:
        ...
        # Similar implementation to 2captcha but with Capsolver API
    ...
    def _solve_perimeterx(self, page: Page, selector: str) -> bool:
        ...
        Quick movements, direct approach, minimal delays.
        """
        ...
        try:
            ...
            # Hold duration 4-6 seconds with fast micro-movements
    ...
    def _human_mouse_move(self, page: Page, target_x: float, target_y: float, steps: int = 25):
        """
...
class WebScraper:
    def __init__(self, config_path="config/config.json", proxies=None):
        ...
        # Initialize CAPTCHA solver (NEW)
        captcha_config = self.config.get("captcha", {})
        self.captcha_solver = CaptchaSolver(captcha_config)
        if self.captcha_solver.enabled:
            logger.info(f"🔓 CAPTCHA solver initialized (service: {captcha_config.get('service', '2captcha')})")
        else:
            logger.debug("🔒 CAPTCHA solving disabled (no API key or disabled in config)")
                ...
                with sync_playwright() as p:
                    ...
                    for i, strategy in enumerate(strategies):
                        try:
                            ...
                            # Try to solve any CAPTCHA present (uses improved solver)
                            captcha_solved = self.captcha_solver.solve(page, url)
                            if captcha_solved:
                                logger.info("CAPTCHA solved successfully, waiting for page update...")
                                try:
                                    page.wait_for_load_state('networkidle', timeout=30000)
                                except Exception:
                                    pass
                            elif self._handle_press_and_hold(page):
                                # Fallback to legacy Press & Hold handler
                                try:
                                    page.wait_for_load_state('networkidle', timeout=30000)
                                except Exception:
                                    pass
